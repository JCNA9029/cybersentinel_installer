...
"""gui.py — CyberSentinel v1 Desktop GUI Run: python gui.py Requires: pip install PyQt6 All existing modules (analysis_manager, lolbas_detector, etc.) are imported directly."""
...
class CyberSentinelGUI(QMainWindow):
    ...
    def _show_privacy_notice(self):
        """One-time first-run privacy disclosure explaining that only file hashes (never file content) are submitted to cloud APIs."""
    ...
    def _show_quarantine_manager(self):
        """Quarantine Manager (Restore / Delete) Displays a dialog to manage encrypted quarantined files."""
    ...
    def _register_gui_callbacks(self, console: "ConsoleWidget"):
        """Set headless_mode=False and attach Qt dialog callbacks so that _prompt_quarantine and engine selection use dialogs instead of input()."""
    ...
    def _gui_feedback_dialog(
        ...
        """Shows an inline post-scan analyst feedback dialog immediately after a malicious verdict while the scan context (file_path) is still available."""
        import threading
 ...
 # badges ──
    ...
    def _build_intel_viewer_page(self):
        """Browseable, searchable viewer for all four cached threat intel feeds: Tab 1 — LOLBAS (intel/lolbas.json) 232 LOLBin entries Tab 2 — Feodo C2 IPs (intel/feodo_blocklist.json) botnet C2 IPs Tab 3 — JA3 Hashes (intel/ja3_blocklist.csv) TLS fingerprints Tab 4 — LOLDrivers (intel/loldrivers.json) vulnerable drivers"""
        import os as _os
        page = QWidget()
        layout = QVBoxLayout(page)
        layout.setContentsMargins(0, 0, 0, 0)
