"""siem_export.py — CyberSentinel SIEM Exporter ============================================= Exports CyberSentinel detections and telemetry to Splunk (or any SIEM that accepts Splunk-compatible HEC JSON) and/or a JSON Lines file."""
...
def _fetch_new_rows(table_def: dict, since_ts: str, batch_size: int) -> list[dict]:
    """Return rows newer than since_ts, ordered oldest-first so the state high-water mark can be updated incrementally."""
