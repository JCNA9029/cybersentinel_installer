...
def _monitor_dns(dga: DgaMonitor):
    """Sniffs UDP port 53 via scapy and feeds every outbound DNS query name into DgaMonitor.analyse()."""
    try:
...
def start_daemon(target_dir: str, webhook_url: str = "", webhooks: dict | None = None):
 ...
 #  DNS sniffer thread feeds DgaMonitor.analyse() — previously DGA was
