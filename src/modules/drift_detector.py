...
class DriftDetector:
    """Monitors the LightGBM model's confidence score distribution over time and raises a drift alert when the distribution shifts significantly."""
    ...
    def _restore_state_from_db(self):
        """Replays ml_score_log to reconstruct the PH in-memory state after a process restart."""
        try:
            with sqlite3.connect(utils.DB_FILE) as conn:
 ...
 # Page-Hinkley running mean
    ...
    def _check_mean_drop(self) -> dict | None:
        """Secondary drift check: compares the rolling mean of the last DETECTION_WINDOW scores against the reference window mean."""
        try:
            with sqlite3.connect(utils.DB_FILE) as conn:
