...
class AdaptiveLearner:
    ...
    def reject_conflicted(self, queue_id: int, reason: str = "") -> bool:
        """Analyst rejects a CONFLICTED correction — it is permanently revoked and will never be used for training."""
        now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    def revoke_correction(self, queue_id: int) -> dict:
        """Revokes a correction at any stage: - PENDING_REVIEW / PENDING / CONFLICTED → marked REVOKED, never trains - TRAINED → model rolled back to the backup snapshot from that session"""
        now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        result = {
            "revoked": False,
    ...
    def _rollback_model(self, session_id: str) -> dict:
        """Restores the model from the backup created immediately before the given session_id ran. Returns a status dict."""
